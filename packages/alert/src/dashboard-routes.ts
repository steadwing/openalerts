...
import type { OpenAlertsEngine, OpenAlertsEvent, AlertEvent } from "@steadwing/openalerts-core";
...
function getRuleStatuses(engine: OpenAlertsEngine): Array<{ id: string; status: "ok" | "fired" }> {
...
function readOpenClawLogs(maxEntries: number, afterTs?: string): ParsedLogEntry[] {
...
export function createDashboardHandler(getEngine: () => OpenAlertsEngine | null): HttpHandler {
  return async (req: IncomingMessage, res: ServerResponse): Promise<boolean> => {
    ...
    if ((url === "/openalerts" || url === "/openalerts/") && req.method === "GET") {
      if (!engine) {
    ...
    if (url.startsWith("/openalerts/logs") && req.method === "GET") {
      ...
      const entries = readOpenClawLogs(limit, afterTs);
