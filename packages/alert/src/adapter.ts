...
export function translateOpenClawEvent(
  ...
  if (event.type === "model.usage") {
    ...
    if (usage) {
      if (typeof usage.totalTokens === "number") {
        ...
        base.tokenCount = ((usage.inputTokens as number) ?? 0) + ((usage.outputTokens as number) ?? 0);
      }
      if (typeof usage.costUsd === "number") {
        base.costUsd = usage.costUsd;
      }
    }
    if (typeof event.tokenCount === "number") base.tokenCount = event.tokenCount;
  ...
  if (event.type === "queue.lane.enqueue" || event.type === "queue.lane.dequeue") {
...
export function translateAgentEndHook(data: {
  ...
  return {
    ...
    error: data.error,
    meta: { messageCount: data.messages?.length ?? 0, source: "hook:agent_end" },
  };
}
...
export class OpenClawAlertChannel implements AlertChannel {
  ...
  async send(alert: AlertEvent, formatted: string): Promise<void> {
    ...
    const opts = this.target.accountId ? { accountId: this.target.accountId } : {};
    ...
    const channelMod = channel[this.target.channel] as Record<string, unknown> | undefined;
    const sendFn = channelMod?.[methodName] as
      | ((to: string, text: string, opts?: Record<string, unknown>) => Promise<unknown>)
      | undefined;
...
export function resolveAlertTarget(
  ...
  const channelKeys = ["telegram", "discord", "slack", "whatsapp", "signal"] as const;

  for (const channelKey of channelKeys) {
    const channelConfig = (cfg as Record<string, unknown>)[channelKey];
    if (!channelConfig || typeof channelConfig !== "object") continue;

    const target = extractFirstAllowFrom(channelKey, channelConfig as Record<string, unknown>);
...
export function parseConfig(raw: Record<string, unknown> | undefined): MonitorConfig {
  if (!raw) return {};
  return {
